...
19. For long-running optimization efforts, bootstrap a campaign directive and compact ledger with [`scripts/campaign_bootstrap.sh`](scripts/campaign_bootstrap.sh).
...
- [`references/campaign-loop.md`](references/campaign-loop.md) for long-running optimization campaigns that need a human-editable directive and a compact keep/discard ledger across many runs.
...
- Before risky edits, checkpoint the last known-good implementation when rollback would otherwise be reconstructive or easy to lose. After serious positive, negative, blocked, or surprising results, checkpoint what was learned immediately. If the current implementation is costly to recreate, strategically important, correct but not yet winning, or a plausible fallback or oracle, preserve it on its own branch or worktree before moving on. Use [`references/checkpointing.md`](references/checkpointing.md) when the preservation decision is unclear.
