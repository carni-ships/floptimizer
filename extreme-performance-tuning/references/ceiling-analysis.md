...
3. Identify the resource ceiling: core saturation, memory bandwidth, disk IOPS, network latency, pool size, queue depth, or dependency fan-out.
4. Convert that into a realistic target, not just a theoretical maximum.
5. Decide whether this path is still worth pursuing.
...
- Continue when the ceiling is large enough and the path is still credible.
